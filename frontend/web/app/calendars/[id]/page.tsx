...
export async function generateMetadata({ params }: CalendarPageProps): Promise<Metadata> {
  ...
  try {
    const detail = await fetchCalendar(id);
    return {
      title: detail.data.name,
      description: detail.data.description?.trim().slice(0, 160) || "Public NIMNear calendar",
    };
  } catch {
...
export default async function CalendarPage({ params }: CalendarPageProps) {
  ...
  return <div className="min-h-svh bg-background"><AppHeader /><main className="mx-auto max-w-[1120px] space-y-8 px-4 py-8 sm:px-6 sm:py-10 lg:px-8"><Link href="/calendars" className="inline-flex items-center gap-2 text-xs font-medium text-muted hover:text-foreground"><ArrowLeft size={14} /> Calendars</Link><CalendarHero calendar={detail.data} /><section className="space-y-4"><div><p className="text-xs font-medium uppercase tracking-[0.16em] text-accent">Event feed</p><h2 className="mt-1 text-xl font-semibold text-foreground">Events in this calendar</h2></div><EventTimeline events={detail.events} /></section></main></div>;
}
